...
CAnimationToolView* g_pMainView;
...
BOOL CAnimationToolApp::OnIdle(LONG lCount)
{
	...
	{
		m_pTimer_Manager->Compute_Timer(L"Timer_Default");
		_float fTimeDelta_default = m_pTimer_Manager->Get_TimeDelta(L"Timer_Default");

		if (m_pFrame_Manager->Permit_Call(L"Frame_60", fTimeDelta_default))
		{
			...
			g_pMainView->TimeUpdate(fTimeDelta);
...
HRESULT CAnimationToolApp::Ready_FrameTimer()
{
	/* Add Timer */
	...
	if (FAILED(m_pTimer_Manager->Add_Timer(L"Timer_Default", pTimer)))
		goto exception;
	...
	/*Add Frame*/
	if (FAILED(m_pFrame_Manager->Add_Frame(L"Frame_60", 60.f)))
		goto exception;

	return NOERROR;

exception:
	AfxMessageBox(L"Ready FrameTimer Failed");
	return E_FAIL;
