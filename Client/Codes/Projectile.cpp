...
void CProjectile::Use_Weapon_Monster(const _float fTimeDelta, const _vec3 & vSourPos, const _vec3 & vDestPos)
{
	...
	if (m_tInfo.m_fFireRateTimer > m_tInfo.m_fFireRate)
	{
		...
		for (int i = 0; i < 3; ++i)
		{
			CProjectileSkill* pSkill = CProjectileSkill::Create(m_pGraphicDev);
			if (nullptr == pSkill)
				return;
		...
		float fTime = sqrtf(-vDir.y / GRAVITY*0.5f);
			...
			vDestPosition.x += ((i-1)*10.f);
			vDestPosition.z += ((i - 1)*10.f);


			fTime = D3DXVec3Length(&vDir) / 10.f;
			pSkill->Set_Info(vDestPosition, vSourPos, fTime, m_tInfo.m_fBulletSpeed, 3.f, m_tInfo.m_iDamage);
...
HRESULT CProjectile::Ready_Component(void)
{
...
exception:
	MSG_BOX("CProjectile Component Ready Failed err: 13115");
	return E_FAIL;
}

CProjectile * CProjectile::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CProjectile *	pInstance = new CProjectile(pGraphicDev);

	if (FAILED(pInstance->Ready_GameObject()))
	{
		MSG_BOX("CProjectile Created Failed");
		Engine::Safe_Release(pInstance);
	}
	return pInstance;
}

_ulong CProjectile::Free(void)
{
	_ulong		dwRefCnt = 0;
