...
HRESULT CQuickSlot::Ready_GameObject(const _float& fDgreeX, const _float& fDgreeY, const _float& fSizeX, const _float& fSizeY)
{
...
HRESULT CQuickSlot::Ready_component()
{
	...
	/*add Cube buffer*/
	pComponent = m_pBufferCom = dynamic_cast<CRectUI_Texture*>(CComponent_Manager::GetInstance()->Clone_Component(SCENE_STATIC, L"Component_Buffer_RectStageUI1"));
	...
	/*Add Texture*/
	pComponent = m_pTextureCom = dynamic_cast<CTexture*>(CComponent_Manager::GetInstance()->Clone_Component(SCENE_STAGE, L"Component_Texture_StageReserveUI"));
	if (nullptr == pComponent)
		goto exception;
...
void CQuickSlot::Set_ChangeItem(_bool Equip)
{
	m_bEquip = Equip;
	if (m_bEquip)
	{
...
CQuickSlot * CQuickSlot::Create(LPDIRECT3DDEVICE9 pGraphicDev, const _float& fDgreeX, const _float& fDgreeY, const _float& fSizeX, const _float& fSizeY)
{
	CQuickSlot*	pInstance = new CQuickSlot(pGraphicDev);
	if (FAILED(pInstance->Ready_GameObject(fDgreeX, fDgreeY, fSizeX, fSizeY)))
	{
		MSG_BOX("CQuickSlot Created Failed");
		Engine::Safe_Release(pInstance);
	}
	return pInstance;
}

_ulong CQuickSlot::Free()
{
	_ulong		dwRefCnt = 0;
